...
?>
<?php
error_log("Translation requested by " . $_SERVER['REMOTE_ADDR']);
$request = file_get_contents('php://input');
parse_str($request, $pars);
error_log("Request: " . $request);
$srclang="";
if (array_key_exists("sourceLanguage", $pars))
{
    $srclang=$pars["sourceLanguage"];
}
$dstlang="";
if (array_key_exists("destinationLanguage", $pars))
{
    $dstlang=$pars["destinationLanguage"];
}
$text="";
if (array_key_exists("text", $pars))
{
    $text=$pars["text"];
}
$trans = json_encode(GoogleTranslate::staticTranslate($text, $srclang, $dstlang));
$response = '{' .
'"sourceLanguage":"' . $srclang . '",' .
'"translation":' . $trans . ',' .
'"status":200,' .
'"destinationLanguage":"' . $dstlang . '",' .
'"providerMessage":"Translations by Google Translate"' .
'}';
